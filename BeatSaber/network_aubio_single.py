...
def main(cuda=torch.cuda.is_available(), gpu=0):
    ...
    for idx, file in enumerate(files):
        with open(file, 'rb') as f:
            ...
            with open('./output_infer/' + str(time) + '.pkl', 'wb') as f:
                ...
                label[label == label.max()] = 1
                label[label < 1] = 0
                print(sum(sum(sum(sum(label)))))
                pkl.dump({'name':file, 'time':time, 'window':window, 'label':label}, f)
